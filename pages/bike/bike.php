          ...
          <li>
            ...
            <select name="location" id="location" style="background-color: #0C0402; color: white; border: none">
              ...
              while ($row = mysqli_fetch_array($store)) {
                if ($row['Address'] != $_SESSION['location']) { ?>
        ...
        <?php if (isset($location) && !empty($location) || isset($_SESSION['location']) && !empty($_SESSION['location'])) { ?>
          <div class="content">
            <div class="content__product">
              <?php while ($row = mysqli_fetch_array($bicycle)) { ?>
                <div class="item item1">
                  <div class="item__img">
                    <a href="./detail.php?name=<?php echo $row['UniqueName'] ?>">
                      <img src="../../assets/homethumb.jpg" alt="" />
                    </a>
                    <a href="./detail.php?name=<?php echo $row['UniqueName'] ?>" class="rentBtn">Rent Now</a>
                  </div>
                  <p class="title"><?php echo $row['UniqueName'] ?></p>
                </div>
              <?php } ?>
            </div>
          </div>
      ...
      <div class="location__container">
        <div class="location">
          <select name="" id="" class="select-location">
            ...
            while ($row = mysqli_fetch_array($store)) {
            ?>
              <option value=""><?php echo $row['UniqueName'] ?></option>
            <?php } ?>
          </select>
        </div>
      </div>
