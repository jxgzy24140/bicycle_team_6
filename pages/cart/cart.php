...
<body>
    ...
    if (!empty($location) && isset($_SESSION['cart'])) {
        $store = StoreDAO::getStoreByAddress($location);
        ...
        foreach ($_SESSION['cart'] as $key => $value) {
            ...
            // { 
            //     $UniqueNameStore = $row['UniqueName'];
            // }
            ...
            $count = $storeDAO->getNumberOfBicycleBelongToSpecificModel($key);
            // $result = mysqli_query($conn, $query);
            // while ($result_row = mysqli_fetch_array($result)) {
            //     $count = $result_row['COUNT(bicycle.IdentifyNumber)'];
            // }
                            ...
                            <table class="table" style="width: 100%; text-align:center; margin-top: 10px">
                                ...
                                <tbody>
                                    <?php foreach ($_SESSION['cart'] as $key => $value) {
                                        ...
                                        $bicycleModel = BicycleModelDAO::getBicycleModel($key);
                                        // while ($row = mysqli_fetch_array($bicycle)) {
                                        // $bike = $row['UniqueName'];
                                        $bike = $bicycleModel->uniqueName;
                                        $location = $_SESSION['location'] ?>
                                        <tr>
                                            <input id="bike" type="text" style="display: none" value="<?php echo $bike; ?>">
                                            <td><?php echo $bicycleModel->uniqueName ?></td>
                                            <td><?php echo $bicycleModel->type ?></td>
                                            <td><?php echo $bicycleModel->gear ?></td>
                                            <td>
                                                <input name="<?php
                                                                // while ($row = mysqli_fetch_array($name_store)) {
                                                                //     $UniqueNameStore = $row['UniqueName'];
                                                                ...
                                                                //                     $count = $result_row['COUNT(bicycle.IdentifyNumber)'];
                                                                //                 }
                                                                $count = $storeDAO->getNumberOfBicycleBelongToSpecificModel($bicycleModel->uniqueName);
                                                                if ($count > 0) {
                                                                    echo $key;
                                                                }
